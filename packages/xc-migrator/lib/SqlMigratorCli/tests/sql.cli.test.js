...
describe("SqlMigratorCli : Tests", function() {
  ...
  it(`xmigrator migration up should pass secondary`, async function() {
    await TestUtil.cmdMigrationUp({
      args: ["m", "u"],
      dbAlias: "secondary"
    });
  ...
  it(`xmigrator clean --env=test --dbAlias=secondary should pass`, async function() {
    this.timeout(20000);
    ...
    for (let i = 0; i < project.envs[key].length; ++i) {
      ...
      if (connection.meta.dbAlias === "secondary") {
        try {
